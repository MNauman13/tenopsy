...
function Hero() {
    ...
    return (
        <div className='flex items-center flex-col mt-20'>
            ...
            <div className="grid w-full max-w-xl mt-5 gap-6 bg-white z-10 border-2 rounded-xl">
                <InputGroup>
                    <InputGroupTextarea
                        data-slot="input-group-control"
                        className="flex field-sizing-content 
                        min-h-24 w-full resize-none rounded-xl 
                        px-3 py-2.5 text-base transition-[color,box-shadow] outline-none md:text-sm"
            ...
            <div className='flex gap-5 mt-5 max-w-3xl flex-wrap justify-center z-10'>
                {QUICK_VIDEO_SUGGESTIONS.map((suggestion, index) => (
                    <h2 key={index} onClick={() => setUserInput(suggestion?.prompt)}
                        className='border rounded-2xl cursor-pointer px-2 p-1 text-sm bg-white/70'>{suggestion.title}</h2>
                ))}
            </div>
        </div>
    )
